...
{% set stg_table_exists_query = "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'dbt-dimensions' AND table_name = 'inc_clients_stg_new')" %}
...
SELECT
    ...
    LEFT JOIN {{source('dbt-dimensions', 'inc_clients_dimension')}} dim on stg.clientid = dim.clientid
