...
{% set stg_table_exists_query = "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'dbt-dimensions' AND table_name = 'inc_clients_stg')" %}
...
{% if is_incremental() and table_exists and stg_table_exists %}
    WHERE (src._airbyte_emitted_at::timestamptz AT TIME ZONE 'UTC' + INTERVAL '3 hours') 
            > COALESCE((SELECT max(loaddate::timestamptz) FROM {{ source('dbt-dimensions', 'inc_clients_dimension') }}), '1900-01-01'::timestamp)
{% endif %}
